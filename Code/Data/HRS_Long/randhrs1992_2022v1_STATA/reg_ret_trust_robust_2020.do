*----------------------------------------------------------------------
* reg_ret_trust_robust_2020.do
* Cross-sectional OLS of 2022 returns on 2020 trust variables WITH RISK CONTROLS (Longitudinal dataset)
* Runs each return measure on each trust variable, with and without controls
* Includes risky_share_2020 as control for risk preferences
* Returns: r_annual_2022, r_annual_trim_2022, r_annual_excl_2022, r_annual_excl_trim_2022
* Controls: r15agey_b, ragender, raedyrs, r15inlbrf, married_2020, born_us, i.race_eth, risky_share_2020, wealth deciles (wealth_d2..wealth_d10 if present)
*----------------------------------------------------------------------
clear all
capture log close
cd "/Volumes/SSD PRO/Github-forks/Chp2-BeliefsandTrust/Code/Data/HRS_Long/randhrs1992_2022v1_STATA"
log using "reg_ret_trust_robust_2020.log", replace text
...
* Check for risky_share_2020 variable
di as txt "=== Checking for risky_share_2020 variable ==="
capture confirm variable risky_share_2020
if _rc {
    di as warn "WARNING: risky_share_2020 not found; will skip in regressions"
    local risky_share_var ""
}
else {
    local risky_share_var "risky_share_2020"
    quietly count if !missing(risky_share_2020)
    di as txt "  risky_share_2020 found: " r(N) " non-missing observations"
}
di as txt ""

* Controls
local ctrl_core "r15agey_b ragender raedyrs r15inlbrf married_2020 born_us"
local ctrl_deciles ""
forvalues d = 2/10 {
...
* Also exclude ragender from ctrl_in since it's handled as factor variable i.ragender
...
if "`risky_share_var'" != "" {
    di as txt "Risky asset share (2020) included: `risky_share_var'"
}
di as txt ""
...
foreach t of local trust_vars {
    ...
    * With controls (including risky_share_2020)
    foreach y of local ret_vars {
        capture confirm variable `y'
        if _rc continue
        quietly count if !missing(`y') & !missing(`t')
        ...
        * Shorten model name to avoid Stata's 32-char limit with _est_ prefix
        ...
        eststo `mname': regress `y' c.`t' c.`t'#c.`t' `age_quad' i.ragender `race_factor' `risky_share_var' `ctrl_in' if !missing(`y') & !missing(`t'), vce(robust)
    ...
    * Export table for this trust variable
    ...
    local outfile "`tables'/trust_`t'_returns_robust.tex"
    ...
    if "`models_raw'`models_ctl'" != "" {
        ...
        * Short row names (LaTeX-safe)
        ...
        local vlab13 risky_share_2020 "Risky share"
        ...
        esttab `models_raw' `models_ctl' using "`outfile'", replace ///
            ...
            varlabels(`vlab' `vlab2' `vlab3' `vlab4' `vlab5' `vlab6' `vlab7' `vlab8' `vlab9' `vlab10' `vlab11' `vlab12' `vlab13') ///
            stats(N r2_a, labels("Observations" "Adj. R-squared")) ///
            title("Returns (2022) on Trust `t' (with risk controls)") ///
            addnote("Robust SEs in parentheses; Age entered quadratically when available." "Controls (when included): raedyrs, in labor force, married, born in U.S." "Includes risky asset share (2020) as control for risk preferences.")
    }
    else {
    ...
    * With controls (including risky_share_2020)
    foreach y of local ret_vars {
        ...
        * Shorten model name as above
        ...
        eststo `mname': regress `y' c.trust_pca1_z c.trust_pca1_z#c.trust_pca1_z `age_quad' i.ragender `race_factor' `risky_share_var' `ctrl_in' if !missing(`y') & !missing(trust_pca1_z), vce(robust)
        local models_pca_ctl "`models_pca_ctl' `mname'"
    }

    * Export PCA table
    local outfile_pca "`tables'/trust_pca_returns_robust.tex"
    ...
    if "`models_pca_raw'`models_pca_ctl'" != "" {
        ...
        * Short row names (LaTeX-safe)
        ...
        esttab `models_pca_raw' `models_pca_ctl' using "`outfile_pca'", replace ///
            ...
            varlabels(`vlab' `vlab2' `vlab3' `vlab4' `vlab5' `vlab6' `vlab7' `vlab8' `vlab9' `vlab10' `vlab11' `vlab12' `vlab13') ///
            stats(N r2_a, labels("Observations" "Adj. R-squared")) ///
            title("Returns (2022) on Trust PC1 (with risk controls)") ///
            addnote("Robust SEs in parentheses; Age entered quadratically when available." "Controls (when included): raedyrs, in labor force, married, born in U.S." "Includes risky asset share (2020) as control for risk preferences." "PC1 variance prop = `pc1_str'")
    }
    else {
...
foreach t of local trust_vars {
    ...
    * Export table for this trust variable
    local tables "/Volumes/SSD PRO/Github-forks/Chp2-BeliefsandTrust/Paper/Tables"
    local outfile "`tables'/trust_`t'_returns_avg_robust.tex"
    di as txt "Exporting LaTeX table: `outfile'"
    if "`models_raw'`models_ctl'" != "" {
        * Short, human column titles for 8 models
        ...
        * Short row names (LaTeX-safe)
        ...
        local vlab13 risky_share_2020 "Risky share"
         ...
         esttab `models_raw' `models_ctl' using "`outfile'", replace ///
             ...
             varlabels(`vlab' `vlab2' `vlab3' `vlab4' `vlab5' `vlab6' `vlab7' `vlab8' `vlab9' `vlab10' `vlab11' `vlab12' `vlab13') ///
             stats(N r2_a, labels("Observations" "Adj. R-squared")) ///
             title("Average Returns (2002-2022) on Trust `t' (with risk controls)") ///
             addnote("Robust SEs in parentheses; Age entered quadratically when available." "Controls (when included): raedyrs, in labor force, married, born in U.S." "Includes risky asset share (2020) as control for risk preferences.")
    }
    else {
        di as warn "No models estimated for trust var `t'; skipping export."
    ...
    * With controls (including risky_share_2020)
    foreach y of local ret_avg_vars {
        ...
        * Shorten model name as above
        ...
        local yshort = subinstr("`yshort'","_","",.)
        local mname = "pca_ctl_`yshort'"
        eststo `mname': regress `y' c.trust_pca1_avg_z c.trust_pca1_avg_z#c.trust_pca1_avg_z `age_quad' i.ragender `race_factor' `risky_share_var' `ctrl_in' if !missing(`y') & !missing(trust_pca1_avg_z), vce(robust)
        local models_pca_ctl "`models_pca_ctl' `mname'"
    }

    * Export PCA table
    local outfile_pca_avg "`tables'/trust_pca_returns_avg_robust.tex"
        ...
        * Short row names (LaTeX-safe)
        ...
        local vlab13 risky_share_2020 "Risky share"
         ...
         esttab `models_pca_raw' `models_pca_ctl' using "`outfile_pca_avg'", replace ///
             ...
             varlabels(`vlab' `vlab2' `vlab3' `vlab4' `vlab5' `vlab6' `vlab7' `vlab8' `vlab9' `vlab10' `vlab11' `vlab12' `vlab13') ///
             stats(N r2_a, labels("Observations" "Adj. R-squared")) ///
             title("Average Returns (2002-2022) on Trust PC1 (with risk controls)") ///
             addnote("Robust SEs in parentheses; Age entered quadratically when available." "Controls (when included): raedyrs, in labor force, married, born in U.S." "Includes risky asset share (2020) as control for risk preferences." "PC1 variance prop = `pc1_str_avg'")
    }
    else {
        di as warn "No PCA models estimated; skipping export."
    }
