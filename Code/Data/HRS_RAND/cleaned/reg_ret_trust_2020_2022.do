...
clear all
capture log close
log using "/Volumes/SSD PRO/Github-forks/Chp2-BeliefsandTrust/Code/Data/HRS_RAND/cleaned/reg_ret_trust_2022.log", replace text
...
* Trust variables (as provided)
local trust_vars "rv557 rv558 rv559 rv560 rv561 rv562 rv563 rv564"

* Return variables: total returns only (raw, trimmed, final-trim) and excl residential
local ret_total_raw   "r_total_2022 r_total_2022_excl_res"
local ret_total_trim  "r_total_2022_trimmed r_total_2022_excl_res_trim"
local ret_total_ftrim "r_total_2022_final_trim r_total_2022_excl_res_ftrim"

* Quick overlap diagnostics for sample sizes (address low N concern)
quietly count
local N_all = r(N)
di as txt "=== Quick overlap check: r_total_2022 and rv557 ==="
quietly count if !missing(r_total_2022)
di as txt "  Non-missing r_total_2022: " r(N) " of " `N_all'
quietly count if !missing(rv557)
di as txt "  Non-missing rv557:        " r(N) " of " `N_all'
quietly count if !missing(r_total_2022) & !missing(rv557)
di as txt "  Non-missing both:         " r(N) " of " `N_all'
di as txt ""

* Controls constructed in extract/compute controls
* Binary: gender (1=male), employed (1=yes), coupled (1=yes), immigration (1=immigrant)
* Continuous: age, education
* Portfolio shares: share_residential share_realestate share_business share_retirement share_financial share_stocks share_safe
* Wealth decile dummies: wealth_d2-wealth_d10 (omit wealth_d1 as baseline if present)
local ctrl_core "gender employed coupled immigration age education"
* Exclude portfolio shares from controls for this run
local ctrl_shares ""
local ctrl_deciles ""
forvalues d = 2/10 {
...
local ctrl_vars "`ctrl_core' `ctrl_shares' `ctrl_deciles'"

* Report which controls are available
di as txt "Controls to include (if present):"
di as txt "  `ctrl_vars'"

* No helper program; rely on Stata's regression output for robustness and clarity

* Loop over trust variables and returns
di as txt "=== BEGIN REGRESSIONS: TOTAL RETURNS on TRUST (no controls) ==="
foreach t of local trust_vars {
    ...
    foreach y of local ret_total_raw {
    ...
    foreach y of local ret_total_trim {
        capture confirm variable `y'
        if _rc continue
        quietly count if !missing(`y') & !missing(`t')
        if r(N)==0 continue
        di as txt "[TRIM] `y' on `t' + `t'^2 (robust)"
        regress `y' c.`t' c.`t'#c.`t' if !missing(`y') & !missing(`t'), vce(robust)
    }
    foreach y of local ret_total_ftrim {
        capture confirm variable `y'
        if _rc continue
        quietly count if !missing(`y') & !missing(`t')
        if r(N)==0 continue
        di as txt "[FTRIM] `y' on `t' + `t'^2 (robust)"
        regress `y' c.`t' c.`t'#c.`t' if !missing(`y') & !missing(`t'), vce(robust)
    }
}

di as txt "=== BEGIN REGRESSIONS: TOTAL RETURNS on TRUST (+ controls) ==="
...
foreach t of local trust_vars {
    ...
    foreach y of local ret_total_trim {
        ...
        quietly count if !missing(`y') & !missing(`t')
        if r(N)==0 continue
        di as txt "[TRIM+C] `y' on `t' + `t'^2 with controls (robust)"
        regress `y' c.`t' c.`t'#c.`t' `ctrl_in' if !missing(`y') & !missing(`t'), vce(robust)
    }
    foreach y of local ret_total_ftrim {
        capture confirm variable `y'
        ...
        quietly count if !missing(`y') & !missing(`t')
        if r(N)==0 continue
        di as txt "[FTRIM+C] `y' on `t' + `t'^2 with controls (robust)"
        regress `y' c.`t' c.`t'#c.`t' `ctrl_in' if !missing(`y') & !missing(`t'), vce(robust)
    }
}
...
if _rc==0 {
    * Proportion of variance explained by PC1 (display only)
    ...
    quietly count if !missing(r_total_2022) & !missing(trust_pca1_z)
    di as txt "Overlap with r_total_2022: " r(N)

    di as txt "=== REGRESSIONS: TOTAL RETURNS on trust_pca1_z (no controls) ==="
    foreach y of local ret_total_raw {
        capture confirm variable `y'
        if _rc continue
    ...
    foreach y of local ret_total_trim {
        ...
        quietly count if !missing(`y') & !missing(trust_pca1_z)
        if r(N)==0 continue
        di as txt "[TRIM] `y' on trust_pca1_z + trust_pca1_z^2 (robust)"
        regress `y' c.trust_pca1_z c.trust_pca1_z#c.trust_pca1_z if !missing(`y') & !missing(trust_pca1_z), vce(robust)
    }
    foreach y of local ret_total_ftrim {
        capture confirm variable `y'
        ...
        quietly count if !missing(`y') & !missing(trust_pca1_z)
        if r(N)==0 continue
        di as txt "[FTRIM] `y' on trust_pca1_z + trust_pca1_z^2 (robust)"
        regress `y' c.trust_pca1_z c.trust_pca1_z#c.trust_pca1_z if !missing(`y') & !missing(trust_pca1_z), vce(robust)
    }

    di as txt "=== REGRESSIONS: TOTAL RETURNS on trust_pca1_z (+ controls) ==="
    foreach y of local ret_total_raw {
        capture confirm variable `y'
        if _rc continue
        quietly count if !missing(`y') & !missing(trust_pca1_z)
    ...
    foreach y of local ret_total_trim {
        ...
        quietly count if !missing(`y') & !missing(trust_pca1_z)
        if r(N)==0 continue
        di as txt "[TRIM+C] `y' on trust_pca1_z + trust_pca1_z^2 with controls (robust)"
        regress `y' c.trust_pca1_z c.trust_pca1_z#c.trust_pca1_z `ctrl_in' if !missing(`y') & !missing(trust_pca1_z), vce(robust)
    }
    foreach y of local ret_total_ftrim {
        capture confirm variable `y'
        ...
        quietly count if !missing(`y') & !missing(trust_pca1_z)
        if r(N)==0 continue
        di as txt "[FTRIM+C] `y' on trust_pca1_z + trust_pca1_z^2 with controls (robust)"
        regress `y' c.trust_pca1_z c.trust_pca1_z#c.trust_pca1_z `ctrl_in' if !missing(`y') & !missing(trust_pca1_z), vce(robust)
    }
}
else {
